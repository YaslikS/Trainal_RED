...
class AfterExerciseAddWidget extends StatelessWidget {
  const AfterExerciseAddWidget({super.key});
                  ...
                  Column(
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: TextField(
                          onChanged: (value) {
                            print("TextField - before - dad2");
                            if (value == "") return;
                            var safeValue = int.parse(value);
                            if (safeValue < 30 || 130 < safeValue) return;
                            MeasureValuesInheritedWidget.of(context)?.changeDad2(safeValue);
                  ...
                  Column(
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: TextField(
                          onChanged: (value) {
                            print("TextField - before - pulse2");
                            if (value == "") return;
                            var safeValue = int.parse(value);
                            if (safeValue < 40 || 230 < safeValue) return;
                            MeasureValuesInheritedWidget.of(context)?.changePulse2(safeValue);
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          Text(
                            MeasureValuesInheritedWidget.of(context)?.kerdo2.toString() ?? "",
  ...
  Color getColorForKerdo(BuildContext context) {
    int? kerdo = MeasureValuesInheritedWidget.of(context)?.kerdo2;
  ...
  calcKerdo(BuildContext context, int? dad, int? pulse) {
    ...
    MeasureValuesInheritedWidget.of(context)?.changeKerdo2(kerdo);
  }

  getPulse(BuildContext context, int dad){
    int? pulse = MeasureValuesInheritedWidget.of(context)?.pulse2;
    calcKerdo(context, dad, pulse);
  }

  getDad(BuildContext context, int pulse){
    int? dad = MeasureValuesInheritedWidget.of(context)?.dad2;
    calcKerdo(context, dad, pulse);
  }
}
